...
//Comment out line below to allow unit test to run, or define it in Makefile
//#define UNIT_TEST
...
4.7927, 	9.6683, 	10.7910, 	0.4441, 	0.8960,
3.2795, 	-4.4170, 	5.5013, 	0.5961, 	-0.8029,
-7.6592, 	0.3624, 	7.6678, 	-0.9989, 	0.0473,
1.7541, 	-8.7826, 	8.9561, 	0.1959, 	-0.9806,
6.0224, 	-1.3266, 	6.1668, 	0.9766, 	-0.2151,
7.9521, 	-4.6506, 	9.2121, 	0.8632, 	-0.5048 
//266 lines
};
...
{
	int errors=0;
//	cout << "Please enter a tolerance value: ";
//	cin >> toler;
	printf("---------------Test Data-----------------\n");
	printf("TOLERANCE = %7.4lf\n", toler);
	for (int i=0; i<266; i++) {
		Vec ki = { testArr[i*5+0], testArr[i*5+1] };
		Vec ao = { ki[0], ki[1] };
		Flt ret = vecNormalize(ao);
		printf("%3i  %7.4lf  %7.4lf  %8.4lf  %7.4lf  %7.4lf",
			i+1, ki[0], ki[1], ret, ao[0], ao[1] );
///////////////////check values here VVVV
		if (check(ret, testArr[5*i + 2])) {
		    printf(" <------ ERROR LENGTH\n");
		    printf("\t\tActual len: %7.4lf -- Expexted len: %7.4lf\n", 
			    ret, testArr[5*i + 2]);
		    errors++;
		}
		else if (check(ao[0], testArr[5*i + 3])){
		    printf(" <------ ERROR X\n");
		    printf("\t\tActual X: %7.4lf -- Expexted X: %7.4lf\n", 
			    ao[0], testArr[5*i + 3]);
		    errors++;
		}
		else if (check(ao[1], testArr[5*i + 4])){
		    printf(" <------ ERROR Y\n");
		    printf("\t\tActual Y: %7.4lf -- Expexted Y: %7.4lf\n", 
			    ao[1], testArr[5*i + 4]);
		    errors++;
		}
		else
